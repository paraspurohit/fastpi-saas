...
def update_user(
    ...
    UserService().update_user(details=user, current_user=current_user, db=db)
    return JSONResponse(
        content={ResponseKey.DETAIL.value: Message.DETAILS_UPDATED.value},
...
def update_password(
    ...
    UserService().update_password(password=password, db=db, current_user=current_user)
    return JSONResponse(
        content={ResponseKey.DETAIL.value: Message.PASSWORD_UPDATED.value},
        status_code=status.HTTP_202_ACCEPTED,
    )
